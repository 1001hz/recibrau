...
try{
  ...
  if($loginResponse["loggedIn"]){
    $query = "SELECT recipe_name, data, id, updated FROM recipes WHERE username = '".$objPostData->username."' AND id = $objPostData->id";
    $sqlResult = query($query);
    if($sqlResult["success"]){
    
      while($row = mysqli_fetch_array($sqlResult["sqlResult"])) {
        $date = new DateTime($row['updated']);
        $resultParts = Array('name'=>$row['recipe_name'], 'updated'=>$date->format('d/m/Y'), 'id'=>$row['id'], 'data'=>$row['data']);
